...
def count_rest_credit(
    ...
    **端数は丸めずに持つ。** みなし出場の段階で切り捨てると、休憩での不足が1試合分を
    超えて開き、戻った人に取り戻させてしまう。生成は adjusted を四捨五入した値で出場者を
    選ぶ（`PlayerStat.adjusted_rounded`）ので、スコアは整数のまま保てる。その丸めの分、
    実際の欠損は1試合分から ±0.5 試合分未満ずれる（偏りは無い）。

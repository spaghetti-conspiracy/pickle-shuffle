...
class Weights:
    ...
    **優先度2（参加回数の公平）は重みではなくハード制約で担保する。**
    ...
    その枠の内側では、重み付き和が次の順になる（1試合ぶんの差分で比較する）。
        初心者ペアの集中(5) 1,000 > 早すぎるペア重複(1) 500 > 連続不参加(3) 360
    ...
    ただし優先度1のうち早すぎるペア重複(500)だけは、連続不参加(360)より上にある。
    ...
    # 初心者の集中(1,000) より小さくする。これより大きいと、初心者を含む構成で
    ...
    # 初心者が片側だけの試合は育成のための手加減が要るので、初心者どうしを対面に
    # 集められるうちは集め、差のある試合を後回しにする（初心者が慣れる時間も稼げる）。
    # 700 だと早すぎるペア重複(500)などと競合し、10人2面・初心者2で片側だけの試合が
    # 9ラウンド目から出た。1,000 なら 14〜15ラウンド目以降に下がり、ピックルボール
    # 経験者の育成負担のばらつきも 700 のとき以下になる。
    beginner_concentration: int = 1_000

...
3. 出したものを確かめる（health、**DB に届いているか**、門が閉じているか、
   メンバー用画面が配られているか）。`PRODUCTION_DOMAIN` が未設定でも、
   いま出したデプロイの URL に対して確かめる

### 合言葉について

**本番の合言葉は GitHub のどこにも無い。** Vercel の環境変数にしか存在せず、
release ワークフローは**合言葉の要らない経路しか叩かない**。
...
**新しいテーブルを足しただけなら、流さなくてよい。** 起動時に、モデルの
テーブルが揃っているかを一覧で照合し、足りなければそのとき作る。
**列を足した・型を変えた場合は自動では直らない**（テーブルはあるので
「揃っている」と見える）ので、上の移行スクリプトが要る。

逆に移行を先に流しておけば、古いコードは増えた列を無視するだけなので落ちない。
...
**外向けの URL（エイリアス）が指しているデプロイを残し、あとは消す**スクリプトを
用意してある。手で実行する（CI からは呼ばない）。

- `pickle-shuffle.vercel.app`（本番）や `pickle-shuffle-staging.vercel.app`（staging）が
  指しているものは使われているので残す。「環境ごとに最新の1つ」だけで選ぶと、
  ロールバックした直後のように URL が古い方を指しているとき、使われているものを消してしまう
- 消し過ぎないよう、次のものも残す
  - 状態が READY / ERROR / CANCELED 以外のもの（ビルド中・待機中・知らない状態）
  - 環境ごとの最新の READY（`STAGING_DOMAIN` を設定していなくても、今の staging を消さない）
  - 作成から10分以内のもの（staging は出してから数秒後にエイリアスを付けるので、その隙間で消さない）
- 消す直前にも、そのデプロイのエイリアスを取り直し、付いていれば飛ばす
- エイリアスが1つも取れなければ何もしない
- `--dry-run` の出力に、残す理由（どの URL か、最新か、など）が出る

**実行すると、ロールバック先も無くなる。** 「戻したいとき」の手順は前の本番デプロイを
使うが、エイリアスの付いていない古い本番デプロイは消える（最新の1つは残る）。
戻す可能性があるうちは実行しない。
下見で消す側が preview（古い staging）だけなら、ロールバック先には触れないので、
取り込みのたびに実行してよい（`CLAUDE.md` の「取り込みの後」）。
...
`--safe` は**どのドメインからも参照されていないものだけ**を消す。考え方はスクリプトと同じで、
スクリプトはそこに「最新・作成直後・ビルド中を残す」「エイリアスが取れなければ止める」を足してある。

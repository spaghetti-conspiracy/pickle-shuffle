...
//:
//: 7分試合＋1.5分の入れ替えを1周期（510秒）として数えると、1台1周期あたり
//: 25回（試合中）＋15回（終盤）＋30回（入れ替え中）＝70回。12台が3時間なら
//: **25,920 回から 17,800 回ほど**（約3割減）。
//:
//: **代償**: ピックルボールは点数で終わるので、リーダーは時間切れを待たずに
//: 「中断」を押すことが多い。手元の時計はまだ動いているつもりなので、
//: そのとき次のカードに気づくまで最大15秒（平均7.5秒）かかる。
//: リーダーが中断してから次を作って確定するまでの手間がそのぶんを覆うので
//: これで運用する。短くしたければ `POLL_IDLE_MS` を下げるだけでよい。
